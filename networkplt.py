...
class Network(nn.Module):
	...
	def __init__(self, mean, tiny):
		...
		self.register_buffer('mean', torch.tensor(mean.size()).to(device))
		self.mean = mean.clone()
		self.tiny = tiny

	def forward(self, inputs,focal_length,opt,plton):
		'''
		Forward pass.
		...
		sc[:, 2] += self.mean[2]
		if plton:
			sc1=torch.squeeze(sc)
			plt.figure()  
			scplot=((sc1-torch.min(sc1))/(torch.max(sc1)-torch.min(sc1)))
			imgplot = plt.imshow(scplot.permute(1, 2, 0))
			plt.colorbar()
		...
		dsacstar.forward_rgb(
                            ...
                            opt.threshold,
                            focal_length, 
                            float(inputs.size(3) / 2), #principal point assumed in image center
                            float(inputs.size(2) / 2), 
                            opt.inlieralpha,
                            opt.maxpixelerror,
                            8) #Output subsample
		poseestimate=out_pose
